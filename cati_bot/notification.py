...
class NotificationSender:
        ...
        try:
            ...
            if self.telegram_enabled:
                ...
                for chat_id, subscription in self.user_subscriptions.items():
                    ...
                    logger.info(f"Проверка подписки для {chat_id}: монета {symbol} ({coin_match}), тип {signal_term_type} ({signal_type_match})")
                ...
                if not subscribers_found:
                    logger.info(f"Подписчики для {symbol} с типом {signal_term_type} не найдены, отправляем в стандартный чат {CONFIG['telegram_chat_id']}")
    ...
    async def _send_available_coins(self, chat_id: str#!/usr/bin/env python3
# -*- coding: utf-8 -*-
