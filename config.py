...
class SignalAnalyzer:
    ...
    def _generate_comprehensive_signal(self, signal_term_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            min_confidence = CONFIG.get(f"{signal_term_type.lower()}_term_confidence", 0.6)
    ...
    def _calculate_signal_confidence(self, signal_type: str, buy_score: float, sell_score: float,
        ...
        try:
            ...
            signal_count = len(
                [msg for msg in getattr(self, 'temp_messages', []) if signal_type.lower() in msg.lower()])
            if signal_count >= 3:
                confidence += 0.1
            elif signal_count >= 5:
                confidence += 0.2
    ...
    def _get_market_info(self) -> List[str]:
        ...
        try:
            ...
            coin_description = ""
            # Можно добавить описание из конфига, если есть
